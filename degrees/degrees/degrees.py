...
def shortest_path(source, target):
    ...
    # Initialize frontier with initial state being the specific name from CSV
    ...
    while True:
        ...
        # Getting the shallowest node from the frontier (Because its a BFS)
        node = frontier.remove()

        # If goal not yet found, mark node as explored
        explored.append(node.state)

        # Add neighbours to frontier
        for action, state in neighbors_for_person(node.state):
            # If state not currently in frontier and the state has not been explored yet
